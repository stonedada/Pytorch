...
for i in range(1, 65):  # 假设有64个块
    block_path = f"./pred/ResUNet/c001_{i}_pred.tif"  # 替换为您的块路径
...
# 缩放到2048
# canvas = zoom(canvas, (1, 2, 2), order=3)

# 保存组合后的原图像
# tifffile.imwrite("TransUNet_pred.tif", canvas)
# tifffile.imwrite("UTransform_pred.tif", canvas)
tifffile.imwrite("ResUNet_pred.tif", canvas)
